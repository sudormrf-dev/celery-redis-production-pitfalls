...
from patterns.worker_config import ConcurrencyModel, WorkerConfig, WorkerPool, WorkerStats
...
class ThroughputSimulator:
    ...
    def _task_latency_ms(self) -> float:
        ...
        # Serializer overhead
        latency += _MSGPACK_OVERHEAD_MS if self.config.serializer == "msgpack" else _JSON_OVERHEAD_MS
...
def print_table(results: list[dict[str, Any]]) -> None:
    ...
    col_labels = ["Configuration", "Workers", "Backend", "Serial.", "Compress", "Latency(ms)", "Tasks", "Tasks/sec"]
    col_keys = ["config", "concurrency", "backend", "serializer", "compression", "latency_ms", "tasks_simulated", "throughput_per_sec"]

    rows: list[list[str]] = []
    for r in results:
        rows.append([str(r[k]) for k in col_keys])

    col_widths = [max(len(label), max(len(row[i]) for row in rows)) + 2 for i, label in enumerate(col_labels)]

    separator = "  " + "-" * (sum(col_widths) + len(col_widths))
    header = "  " + "".join(label.ljust(col_widths[i]) for i, label in enumerate(col_labels))
...
def print_insights(results: list[dict[str, Any]]) -> None:
    ...
    # Concurrency scaling
    r1 = next(r for r in results if r["concurrency"] == 1 and r["serializer"] == "json" and r["backend"] == "redis")
    r4 = next(r for r in results if r["concurrency"] == 4 and r["serializer"] == "json" and r["backend"] == "redis" and r["compression"] == "none")
    r16 = next(r for r in results if r["concurrency"] == 16 and r["serializer"] == "json" and r["backend"] == "redis")
    print(f"  • 4×  workers vs 1×  : {r4['throughput_per_sec'] / r1['throughput_per_sec']:.1f}x throughput")
    print(f"  • 16× workers vs 1×  : {r16['throughput_per_sec'] / r1['throughput_per_sec']:.1f}x throughput")

    # Backend impact
    r_disabled = next(r for r in results if r["concurrency"] == 4 and r["backend"] == "disabled" and r["serializer"] == "json")
    pct = (r_disabled["throughput_per_sec"] - r4["throughput_per_sec"]) / r4["throughput_per_sec"] * 100
    print(f"  • Disabling result backend (+{pct:.0f}% throughput @ 4 workers)")

    # Serializer impact
    r_msgpack = next(r for r in results if r["concurrency"] == 4 and r["serializer"] == "msgpack" and r["backend"] == "redis")
    ser_pct = (r_msgpack["throughput_per_sec"] - r4["throughput_per_sec"]) / r4["throughput_per_sec"] * 100
    print(f"  • msgpack vs json     : +{ser_pct:.0f}% throughput (lower (de)serialization cost)")

    # Compression cost
    r_gzip = next(r for r in results if r["compression"] == "gzip")
    gz_pct = (r4["throughput_per_sec"] - r_gzip["throughput_per_sec"]) / r4["throughput_per_sec"] * 100
    print(f"  • gzip compression    : -{gz_pct:.0f}% throughput (worthwhile only for large payloads)")

    # Best config
    best = max(results, key=lambda r: r["throughput_per_sec"])
    print(f"  • Best config         : [{best['config'].strip()}] → {best['throughput_per_sec']} tasks/sec")
...
def main() -> None:
    ...
    best_cfg = max(BENCH_CONFIGS, key=lambda c: ThroughputSimulator(config=c, duration_seconds=5.0).run()["throughput_per_sec"])
    ...
    print(f"    BackendConfig : type={bc.backend_type.value}, serializer={bc.result_serializer}")
